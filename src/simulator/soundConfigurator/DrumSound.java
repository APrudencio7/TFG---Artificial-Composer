...
public class DrumSound {
	...
	public void setDrumTimes(int durBeat, int numBars) {
		...
		if (this.drumType == typeOfDrum.HiHatsClosed) {
			// We set the times where the drum will be played.
			for (i=0; i < numBars; i++) {
				for (int k = 0; k < this.numReps; k++)
					this.startTimes.add(Math.round((durBeat*i*4) + (selectRandomBeatMultiplyier(rand)*durBeat*4)));
			}			
		}else {
			// We set the times where the drum will be played.
			for (i=0; i < numBars; i++)
	...
	private float selectRandomBeatMultiplyier(Random rand) {
		List<Float> sepValues = Arrays.asList(0f, 0.125f, 0.25f, 0.375f, 0.5f, 0.625f, 0.75f, 0.875f);
		
		float selectedValue = rand.nextInt(sepValues.size());
		
		if(selectedValue == 0f || selectedValue == 0.25f || selectedValue == 0.5f || selectedValue == 0.75) {
			this.numPrincipalDivs += 1;
		}
		
		return sepValues.get(rand.nextInt(sepValues.size()));
	}
